Ви надіслали запит на виправлення файлів з урахуванням помилки в app.js (відсутній експорт connectWalletUI у wallet.js). Я виправив код:

У app.js: Замінив connectWalletUI на connectWallet (бо саме ця функція експортується з wallet.js). Також додав присвоєння window.connectWallet = connectWallet; для глобальної доступності (щоб кнопка в HTML працювала).
Для всіх файлів (wallet.js, app.js, index.html, contracts.js, config.js): Переклав коментарі з англійської на українську мову. Залишив код без змін (крім виправлення в app.js). Коментарі тепер українською, щоб відповідати вашому запиту. Якщо коментарі були багаторядкові (/** */), переклав їх повністю.
У index.html: Коментарі (якщо є) також переклав, але там їх мало (в основному HTML/JS).

Ось виправлені файли як текст (у форматі <DOCUMENT>). Ви можете скопіювати їх і замінити оригінали. Якщо потрібно, протестуйте локально.
